...
@Log4j2
public class DeleteCommentLikeTest extends BaseSetting {
    @Autowired
    ...
    public void deleteCommentLikeTest() throws Exception {
        ...
        resultActions=mockMvc.perform(MockMvcRequestBuilders.post("/api/albumchat/commentlike/create")
                        .header("Authorization", "Bearer " + validToken)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        content = result.getResponse().getContentAsString();
        AlbumChatCommentLikeIdResponse albumChatCommentLikeReq = mapper.readValue(content, AlbumChatCommentLikeIdResponse.class); // 응답 JSON을 PostDto 객체로 변환
        Long albumChatCommentLikeId = albumChatCommentLikeReq.getAlbumChatCommentLikeId();

        mockMvc.perform(MockMvcRequestBuilders.post("/api/albumchat/commentlike/delete")
                        .header("Authorization", "Bearer " + validToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(AlbumChatCommentLikeIdResponse.builder()
