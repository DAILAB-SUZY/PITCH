...
public class DeletePostLikeTest extends BaseSetting {
    ...
    public void deleteLikeTest() throws Exception {
        ...
        url="/api/like/create";
        resultActions=mockMvcHelper(HttpMethod.POST,url,likeDto,userLogin.getToken()).andExpect(status().isOk());
        result = resultActions.andReturn();
        ...
        url="/api/like/delete";
        mockMvcHelper(HttpMethod.POST,url,likereq1,userLogin.getToken()).andExpect(status().isOk());
    }
}
