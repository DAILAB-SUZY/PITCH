...
/**
 * 댓글 관련 API를 제공하는 REST 컨트롤러입니다. 댓글 조회, 생성, 수정, 삭제 기능을 제공합니다.
 */
@RestController
...
public class CommentApi {
  ...
  public ResponseEntity<List<CommentDetail>> createComment(
      @RequestBody CreateCommentRequest comment, @PathVariable Long postId,
      @AuthenticationPrincipal Long userId) {
    return ResponseEntity.ok(
        commentService.createComment(comment.getParent_id(), comment.getContent(), postId, userId));
  }
  ...
  public ResponseEntity<List<CommentDetail>> updateComment(
      @RequestBody UpdateCommentRequest comment, @PathVariable Long commentId,
      @AuthenticationPrincipal Long userId) {
