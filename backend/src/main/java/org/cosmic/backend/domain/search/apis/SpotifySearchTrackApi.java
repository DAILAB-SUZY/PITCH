...
public class SpotifySearchTrackApi {
    ...
    public ResponseEntity<List<ArtistTrackResponse>> searchTrackByArtistIdAndTrackName(
            @Parameter(description = "앨범 이름")
            @PathVariable String artistId,@PathVariable String trackName) throws JsonProcessingException { //q는 검색어
        return ResponseEntity.ok(searchTrackService.searchTrackByArtistIdAndTrackName(createSpotifyToken.accesstoken(),artistId,trackName));
    }
