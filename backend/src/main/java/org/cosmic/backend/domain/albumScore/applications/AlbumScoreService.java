...
public class AlbumScoreService {
  ...
  public AlbumDetail addScore(Long albumId, Long userId, Integer score) {
    ...
    return AlbumDetail.from(
        AlbumScore.from(usersRepository.findByUserId(userId).get(), album, score));
  }
  ...
  public void createAlbumScore(Album album, Long userId, int score) {
    if (albumScoreRepository.existsByAlbum_AlbumIdAndUser_UserId(album.getAlbumId(),
        userId)) {//이미 존재하면
      if (score != 0) {
        albumScoreRepository.findByAlbum_AlbumIdAndUser_UserId(album.getAlbumId(), userId)
            .setScore(score);
      } else {
        albumScoreRepository.delete(
            albumScoreRepository.findByAlbum_AlbumIdAndUser_UserId(album.getAlbumId(), userId));
      }
    } else {//처음 만드는 상황
      if (score != 0) {
        albumScoreRepository.save(
            AlbumScore.from(usersRepository.findByUserId(userId).get(), album, score));
      }
    }
  }
